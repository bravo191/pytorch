...
struct ConcretePyInterpreterVTable final
  ...
  void throw_abstract_impl_not_imported_error(
    ...
    try {
      ...
      if (e.matches(PyExc_NotImplementedError)) {
        throw c10::NotImplementedError(e.what());
      }
      throw;
    }
  }
  ...
  bool fake_try_custom_op_impl(
      const c10::OperatorHandle& op,
      torch::jit::Stack* stack,
      bool* inferred_from_real) const override;
...
DEFINE_CACHED_PYTHON_IMPORT(
    get_run_fake_impl_with_inference_info,
    py::module::import("torch._library.fake_impl")
        .attr("run_fake_impl_with_inference_info"))
DEFINE_CACHED_PYTHON_IMPORT(
    get_is_builtin,
...
bool ConcretePyInterpreterVTable::fake_try_custom_op_impl(
    const c10::OperatorHandle& op,
    torch::jit::Stack* stack,
    bool* inferred_from_real) const {
  py::gil_scoped_acquire gil;
  py::handle py_op = getTorchApiFunction(op);
  auto active = get_active_fake_mode();
  *inferred_from_real = false;
  return run_fake_python_callback(
      op,
      stack,
      [&](const py::object& args, const py::dict& kwargs) -> py::object {
        py::tuple result = get_run_fake_impl_with_inference_info()(
                               active.py_fake_mode, py_op, args, kwargs)
                               .cast<py::tuple>();
        TORCH_INTERNAL_ASSERT(result.size() == 2);
        *inferred_from_real = result[1].cast<bool>();
        return py::reinterpret_borrow<py::object>(result[0]);
      },
      /*convert=*/{},
      "fake_impl");
}
