...
class AOTInductorModelContainer {
  ...
  void set_use_stream_affinity(bool use_stream_affinity) {
    ...
    if (available_models_.size() != models_.size() ||
      ...
      throw std::runtime_error(
          "Stream affinity must be configured before the first model run");
    }
 ...
 private:
  ...
  // Under models_mutex_, each model is in exactly one execution state:
  // available, pending device completion, or checked out by a host thread.
  // Stream bindings are persistent preferences independent of those states;
  // borrowing a bound model preserves its binding, while a failed run clears
  // it so the next call may choose another model.
  ...
  // Stream handles and their locks persist for the container's lifetime.
  // Callers enabling affinity must keep those streams alive to avoid a reused
  // device handle inheriting an old binding.
  ...
  AOTInductorModel* get_stream_affine_model(
    ...
    // If active streams outnumber model instances, reclaim and rebind one.
    // Cardinality-matched benchmark configurations avoid this fallback.
    auto* model = take_available_model(lk, /*prefer_unbound=*/true);
    bind_model_to_stream(model, stream);
    return model;
  }

  std::unique_lock<std::mutex> acquire_stream_lock(DeviceStreamType stream) {
#if defined(USE_CUDA) || defined(USE_XPU)
    ...
    return std::unique_lock<std::mutex>(*stream_mutex);
#else
    (void)stream;
    return {};
#endif
