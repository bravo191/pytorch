...
struct C10_API PyInterpreterVTable {
  ...
  virtual bool fake_try_meta(
      const c10::OperatorHandle& op,
      torch::jit::Stack* stack) const = 0;
  // Try a torch.library fake implementation. inferred_from_real reports that
  // the callback already ran real propagation for its result.
  virtual bool fake_try_custom_op_impl(
      const c10::OperatorHandle& op,
      torch::jit::Stack* stack,
      bool* inferred_from_real) const = 0;
